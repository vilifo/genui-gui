...
function FormikModelForm(props) {
    ...
    return (<Formik
        ...
        >
            {formik => (<Form id={`${props.modelClass}-${props.formNameSuffix}-form`} onSubmit={formik.handleSubmit}
                    ...
                    {!disabled.includes('description') ? (<React.Fragment>
                            <FormGroup>
                    ...
                    {props.enableFileUploads ? (<React.Fragment>
                            <FormGroup>
                    ...
                    {formik.initialValues.hasOwnProperty(trainingStrategyPrefix) || TrainingStrategyExtras ? (
                        <React.Fragment>
                            ...
                            {TrainingStrategyExtras ? <TrainingStrategyExtras
                                {...props}
                    ...
                    {props.chosenAlgorithm.name !== 'QSPRPredScikitModel' ? parameters.map(param => {
                        const name = `${trainingStrategyPrefix}.parameters.${param.name}`;
                        return (<FormGroup key={name} row>
                    ...
                    {formik.initialValues.hasOwnProperty(validationStrategiesPrefix) ? <React.Fragment>
                        {validationStrategiesFields || !disabled.includes(`${validationStrategiesPrefix}.metrics`) ?
                            <h4>Validation Parameters</h4> : null}
                        {validationStrategiesFields ? React.createElement(validationStrategiesFields, {
                            ...props, validationStrategiesPrefix: validationStrategiesPrefix, formikProps: formik
                        }) : null}
                    </React.Fragment> : null}
