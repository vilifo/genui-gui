...
const ProjectManager = React.forwardRef((props, ref) => {
    ...
    const deleteProject = (project, callback) => {
        fetch(new URL(project.id + "/", props.projectListURL), {
            method: 'DELETE',
            credentials: "include"
        }).then((response) => {
            if (response.ok) {
    ...
    return project_id ? (
        <ComponentWithResources
            updateCondition={(prev, now) => prev.project && now.project ? prev.project.id !== now.project.id : false}
            project={project} definition={{project: new URL(project_id + '/', props.projectListURL)}}>
            {
                (isLoaded, data) => {
                    if (isLoaded) {
                        return (
